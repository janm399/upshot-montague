...
abstract class SemanticParseNode[S <: SyntacticLabel[S]](
    val syntactic: S,
    val semantic: SemanticState,
    val exs: Set[ClassCastException]
) {
  ...
  def logicalFormOption[LF: ClassTag]: Option[LF] = {
    ...
    if (implicitly[ClassTag[LF]] == classTag[Nothing])
      throw new IllegalArgumentException("ClassTag not supplied by caller")
    semantic match {
  ...
  def toSvg: String = {
    ...
    val out = (cmd #< is).lazyLines_!
    out
      .mkString("\n")
      .replace(
        "Lambda",
        "&#955;"
      ) // Pretty rendering of Lambdas, useful when FunctionReaderMacro is used.
  }
  ...
  def allNodes: List[SemanticParseNode[S]] =
    this :: children.flatMap(_.allNodes)
...
/**
  ...
  */
case class Terminal[S <: SyntacticLabel[S]](
    s: S,
    m: SemanticState,
    parseToken: ParseToken,
    spans: Spans,
    x: Set[ClassCastException]
) extends SemanticParseNode(s, m, x) {
...
/**
  ...
  * @tparam S The syntactic scheme. In our case, CcgCat
  */
case class NonTerminal[S <: SyntacticLabel[S]](
    s: S,
    m: SemanticState,
    operatorNode: SemanticParseNode[S],
    argumentNode: SemanticParseNode[S],
    x: Set[ClassCastException]
) extends SemanticParseNode(s, m, x) {
  override def toString: String =
    toStringHelp(indent = "  ", withSemantics = true)
  def toStringHelp(
      indent: String = "",
      currentIndent: String = "",
      withSemantics: Boolean = false
    ...
    val operatorString = operatorNode.toStringHelp(
      indent = indent,
      currentIndent = nextIndent,
      withSemantics = false
    )
    val argumentString = argumentNode.toStringHelp(
      indent = indent,
      currentIndent = nextIndent,
      withSemantics = false
    )
    val str =
      s"($spans, '" + s.toString + "', " + operatorString + ", \n" + indent + argumentString + ")"
  ...
  def toDotStringHelp(pre: String): String = {
    val sem =
      meaningString.replaceAll(
        "(\\S),(\\S)",
        "$1, $2"
      ) // Prettify a little by adding spaces after commas.
      ...
      if (rawString.contains(" => ")) {
        val predicate = operatorNode match {
          case nonTerminalNode: NonTerminal[S] =>
            ...
            nonTerminalNode.meaningString
              .split(" => ")
              .drop(1)
              .mkString(" => ")
              .dropRight(1) // pred is everything after first "=>"
          case _ =>
        ...
        if (operatorNode.semantic.toString.contains("Lambda(")) {
          val boundVariable = operatorNode.semantic.toString
            .split("Lambda\\(")(1)
            .split(':')(0)
            .replaceAllLiterally("(", "") // var is between "Lambda(" and ":"
  ...
  def parseTokenString: String =
    node1.parseTokenString + " " + node2.parseTokenString
