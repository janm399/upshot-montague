...
case object Paren extends TerminalCat {
  val category = "Paren"
} // syntactic category for parenthetical expressions
...
object ArithmeticLexicon {
  val lexicon = ParserDict[CcgCat]() +
    (Seq("+", "plus") -> ((N \ N) / N, lambda { y: Int =>
      lambda { x: Int => x + y }
    })) +
    (Seq("-", "minus") -> ((N \ N) / N, lambda { y: Int =>
      lambda { x: Int => x - y }
    })) +
    (Seq("*", "times") -> ((N \ N) / N, lambda { y: Int =>
      lambda { x: Int => x * y }
    })) +
    (Seq("+/-", "plus/minus") -> Seq( // example of ambiguous definition
      ((N \ N) / N, lambda { y: Int => lambda { x: Int => x + y } }),
      ((N \ N) / N, lambda { y: Int => lambda { x: Int => x - y } })
    )) +
    ("(" -> (Paren / N, identity)) +
    (")" -> (N \ Paren, identity)) +
    (IntegerMatcher -> (N, { i: Int =>
      Form(i)
    })) + // IntegerMatcher matches using Integer.parseInt
    (Else -> (X | X, { s: String =>
      identity
    })) // Else matches any single tokens that don't match anything else;
